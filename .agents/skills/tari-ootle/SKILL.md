---
name: tari-ootle
description: Tari Ootle development instructions for building templates (WASM smart contracts) and client applications on the Tari L2 network.
---

# Tari Ootle Development Instructions
...
- `args!` (from `tari_ootle_transaction`) — used **in test code and client code** for `TransactionBuilder`, `TemplateTest::call_function`, and `call_method`. Produces `Vec`.
...
mod my_template {
    ...
    impl MyComponent {
        // CONSTRUCTOR: any function returning Self or Component
...
- Supported field types: all Rust primitives, `String`, `Vec`, `HashMap`, `BTreeMap`, `Option`, `Vault`, `Amount`, `ResourceAddress`, `ComponentAddress`, `RistrettoPublicKeyBytes`, `NonFungibleId`, `ComponentManager`, and any struct inside the `#[template]` module
...
// Explicit constructor — returns Component for full control
pub fn new() -> Component {
...
pub fn new(addr: ComponentAddressAllocation) -> Component {
    Component::new(Self { counter: 0 })
...
// Mint later via ResourceManager
...
let ids: BTreeSet = vault.get_non_fungible_ids();    // All NFT IDs in vault
...
let wasm_binary: Vec = std::fs::read("target/wasm32-unknown-unknown/release/your_template.wasm")?;
...
> **IMPORTANT API note:** On `SubstateId`, the method is `as_template()` — it returns `Option`. There is NO `as_template_address()` method on `SubstateId`. If you need the underlying `TemplateAddress` (a `Hash32`), call `.as_template_address()` on the `PublishedTemplateAddress` result, not on the `SubstateId`.
...
tari_ootle_wallet_cli -d /ip4/127.0.0.1/tcp/12009
...
tari_ootle_wallet_cli transactions submit call-function <template_address> \
    new \
    --fee-account my-account \
    --wait-timeout 30

# Call a component method with arguments
tari_ootle_wallet_cli transactions submit call-method <component_address> \
    guess \
    -a 5 -a <player> \
    --fee-account my-account
...
tari_ootle_wallet_cli transactions get <tx_id>

# Send tokens to another account
tari_ootle_wallet_cli transactions send <amount> <address> \
    --fee-account my-account

# Confidential transfer
tari_ootle_wallet_cli transactions confidential-transfer <amount> <address> \
    --account my-account
```
...
mod counter {
    ...
    impl Counter {
        pub fn new(initial: u64) -> Component {
...
mod guessing_game {
    ...
    impl GuessingGame {
        pub fn new(address: ComponentAddressAllocation) -> Component {
...
8. **Using the wrong `args!` macro** — Ensure you use `args!` from `tari_ootle_transaction` for client/test code (produces `Vec`) and `args!` from `tari_template_lib::prelude` for cross-template calls inside templates.
